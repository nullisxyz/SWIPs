---
swip: <to be assigned>
title: BzzAddress Signature Scheme v1
author: mfw78 (@mfw78)
status: Draft
...
Replace the legacy BzzAddress signature scheme with a simplified v1 format that removes redundant prefixes and uses straightforward concatenation of underlay bytes.
...
This is a breaking change to the signature scheme. A two-release migration is required:

1. **Release N.** Both v0 and v1 signatures are accepted. Nodes generate v1 but verify both.
2. **Release N+1.** Only v1 signatures are accepted. Legacy v0 is rejected.
